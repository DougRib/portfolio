export const BaseInfo = {
    name: "Douglas",
    title: "Full Stack Developer",
    description: "I'm a passionate software engineer with a strong background in JavaScript and TypeScript. I love building clean and maintainable code, and I'm always eager to learn new technologies and techniques.",
    profile:"/images/20231012_114511.jpg"
};

export const AboutInfo = {
    title: "Web application with TypeScript",
    description: "I've been working with Next.js and React for the past few years, focusing primarily on creating dynamic and server-rendered web applications. I love building scalable and maintainable applications that can handle high traffic loads and provide a seamless user experience.",
    client: "10",
    experience: "1",
    project: "1",
    skill: "10"
};

export const ProjectsData = [
    {
        id: 1,
        title: "Portfolio Website",
        image: "/images/p1.png",
        url: "https://github.com/douglasribeiro"
    },
    {
        id: 2,
        title: "E-commerce website",
        image: "/images/p1.png",
        url: "https://github.com/douglasribeiro/ecommerce-nextjs"
    },
    {
        id: 3,
        title: "Landing page",
        image: "/images/p1.png",
        url: "https://github.com/douglasribeiro/landing-page-nextjs"
    },
    {
        id: 4,
        title: "Chat app",
        image: "/images/p1.png",
        url: "https://github.com/douglasribeiro/chat-app-nextjs"
    },
    {
        id: 5,
        title: "Blog website",
        image: "/images/p1.png",
        url: "https://github.com/douglasribeiro/blog-nextjs"
    },
    {
        id: 6,
        title: "API development",
        image: "/images/p1.png",
        url: "https://github.com/douglasribeiro/api-nextjs"
    }
];

export const SkillsData = [
    {
        id: 1,
        title: "Next.js",
        image: "/images/nextjs.svg",
        percent: "40%"
    },
    {
        id: 2,
        title: "React",
        image: "/images/react.svg",
        percent: "70%"
    },
    {
        id: 3,
        title: "TypeScript",
        image: "/images/ts.svg",
        percent: "80%"
    },
    {
        id: 4,
        title: "Python",
        image: "/images/python.svg",
        percent: "25%"
    },
    {
        id: 5,
        title: "Tailwind CSS",
        image: "/images/tailwind.svg",
        percent: "60%"
    },
    {
        id: 6,
        title: "Node.js",
        image: "/images/node.svg",
        percent: "50%"
    },
    {
        id: 7,
        title: "MongoDB",
        image: "/images/mongo.svg",
        percent: "30%"
    },
    {
        id: 8,
        title: "GraphQL",
        image: "/images/ts.svg",
        percent: "20%"
    },
    {
        id: 9,
        title: "AWS",
        image: "/images/ts.svg",
        percent: "10%"
    },
    {
        id: 10,
        title: "Docker",
        image: "/images/ts.svg",
        percent: "15%"
    },
];

export const Blog = [
    {
        id: 1,
        title: "First Blog Post",
        description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed facilisis, dui vitae pharetra commodo, enim justo faucibus est, id fermentum velit neque in neque.",
        image: "/images/blog1.jpg",
        date: "12/02/2022"
    },
    {
        id: 2,
        title: "Second Blog Post",
        description: "Ut id lectus a velit tempus viverra. Duis rutrum, mauris non elementum consectetur, enim diam consectetur nunc, in placerat felis velit eu erat.",
        image: "/images/blog2.jpg",
        date: "11/02/2022"
    }, 
    {
        id: 3,
        title: "Third Blog Post",
        description: "Nullam varius, justo id tincidunt aliquet, justo velit faucibus lectus, ut dignissim mi dolor vitae velit.",
        image: "/images/blog3.jpg",
        date: "10/02/2022"
    }
]

export const ContactData = {
    email: "your-email@example.com",
    phone: "+1 123-456-7890",
    address: "123 Main St, City, State, Zip Code"
};